...
public class UUIDToBase62FunctionFactory implements FunctionFactory {
    ...
    public Function newInstance(final int position, final ObjList<Function> args, IntList argPositions, final CairoConfiguration configuration, SqlExecutionContext sqlExecutionContext) {
        return new UUIDToBase62Func(args.get(0));
    }

    private static class UUIDToBase62Func extends StrFunction implements UnaryFunction {
        private final StringSink sink = new StringSink();
        private final CharBuffer charBuffer = CharBuffer.allocate(11);

        private void putDigit(int index, int charIndex) {
            charBuffer.put(index, UUIDToBase62FunctionFactory.getDigit(charIndex));
        }
        ...
        public void acceptLong(long value) {
            ...
            sink.put(charBuffer, 0, charBuffer.length());
            charBuffer.clear();
        }

        private final Function arg;

        public UUIDToBase62Func(final Function arg) {
            this.arg = arg;
        }
        ...
        public CharSequence getStr(final Record rec) {
            final CharSequence sym = getArg().getStr(rec);
            sink.clear();
            final UUID uuid = UUID.fromString((String) sym);
            acceptLong(uuid.getMostSignificantBits());
            acceptLong(uuid.getLeastSignificantBits());
            return sink;
        }
